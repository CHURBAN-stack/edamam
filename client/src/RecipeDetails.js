...
var warning = {
  ...
  for (let i in healthLabels) {
    ...
    if (warning[f] !== undefined) {
      warning[f].flag = true;
    }
  }

  //Create icon list to print
  var icons = [];
  for (let i in warning) {
    if (warning[i].flag === false) {
      icons.push(warning[i]);
    }
  }

  //console.log(warning.Shellfish-Free.img);
      ...
      <div key={uuidv4()}>
        <Accordion>
          <AccordionSummary
            ...
            id="panella-header"
          >
            <Typography sx={{ fontFamily: "Museo-Sans" }}>
              Ingredients
            </Typography>
          </AccordionSummary>
          <AccordionDetails>
            <Typography component={"span"} sx={{ fontFamily: "Lato" }}>
              <ul key={uuidv4()} className={style.ingredientList}>
                {ingredients.map((ingredient) => (
                  <li key={uuidv4()} className={style.ingredientText}>
                    {ingredient.text}
                  </li>
                ))}
              </ul>
              ...
              <ul key={uuidv4()} className={style.healthLabelsList}>
                {nutriKeys.map((key) => (
                  <li key={uuidv4()}>
                    ...
                    {totalNutrients[key].quantity.toFixed(2)}{" "}
